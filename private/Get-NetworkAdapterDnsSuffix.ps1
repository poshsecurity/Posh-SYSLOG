Function Get-NetworkAdapterDnsSuffix
{
    <#
        .SYNOPSIS
    ...
    # Find the Address
    $LocalAdapter = $LocalAdapters.Where({$_.GetIPProperties().UnicastAddresses.Address.IPAddressToString -contains $IPAddress})

    if ($LocalAdapter.Count -ne 1)
    {
        Throw 'Unable to find correct network adapter'
    }

    $LocalAdapter.GetIPProperties().DnsSuffix
}