...
InModuleScope $script:ModuleName {
    Describe "Basic function unit tests" -Tags Build , Unit{
        It 'Does not accept null IP Address' {
            {Get-NetworkAdapterDnsSuffix -IPAddress $null} | Should Throw
        }

        It 'Does not accept empty string for an IP Address' {
            {Get-NetworkAdapterDnsSuffix -IPAddress ''} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (1)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress 'cat'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (2)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '321.321.321.321'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (3)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '321.123.123.123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (4)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.321.123.123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (5)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.123.321.123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (6)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.123.123.321'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (7)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (8)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (9)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.123.123'} | Should Throw
        }

        It 'Does not accept invalid IP Addresses (10)' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '123.123.123.123.123'} | Should Throw
        }

        It 'Throws an error if it cannot find the IP address' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '1.1.1.1'} | Should Throw
        }

        It 'Does accept a valid ip address' {
            {Get-NetworkAdapterDnsSuffix -IPAddress '127.0.0.1'} | Should Not Throw
        }
    }
